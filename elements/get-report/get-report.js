...
class GetReport extends LitElement {
    ...
    _getReport(event) {
        this.report = event.detail.report;
        this.keys = event.detail.keys.map(item => item._id);
    ...
    _generateReport(report, keys, params) {
        ...
        let url = report.generator === "nodejs" ? "https://raport.cbn-it.ro/" : "/do/Raport";

        let html = "<!DOCTYPE html><html><head><title></title></head><body><form id='formRaport' action='" + url + "' target=\"_blank\" method='GET'>";

        html += "<input type='hidden' name='namespace' value='" + window.data._selectedFirm + "'/>";
